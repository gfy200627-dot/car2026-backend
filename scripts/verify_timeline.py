...
# 5. 市场趋势 months 同步
mt = c.get(f"{BASE}/market/trend", params={"span": 60}, headers=H).json()["data"]
expect("market/trend 时间轴一致", mt["months"] == EXPECT_FULL)

# 6. 地区两端口径一致（span=12）
d = c.get(f"{BASE}/dashboard/region", params={"span": 12}, headers=H).json()["data"]["regions"]
m = c.get(f"{BASE}/market/region", params={"span": 12}, headers=H).json()["data"]
dm = {i["name"]: i["value"] for i in d}
mm = {i["name"]: i["value"] for i in m}
expect("dashboard/market 地区口径一致", dm == mm)
gd = c.get(f"{BASE}/dashboard/region", params={"span": 12}, headers=H).json()["data"]["regions"][0]
...
tr = c.get(f"{BASE}/market/trend", params={"span": 12, "region": "北京市"}, headers=H).json()["data"]
...
rows = c.get(f"{BASE}/sales", params={"brandId": 2, "span": 18, "pageSize": 500}, headers=H).json()["data"]
...
rank = c.get(f"{BASE}/dashboard/car-ranking", headers=H).json()["data"]
sales_page = c.get(f"{BASE}/sales", params={"span": 12, "pageSize": 500, "brandId": 1}, headers=H).json()["data"]
